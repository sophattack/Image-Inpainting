...
class LocalDis(nn.Module):
    def __init__(self, config, mask_shape, use_cuda=True, device_ids=None):
        super(LocalDis, self).__init__()
        self.input_dim = config['input_dim']
        self.cnum = config['ndf']
        self.use_cuda = use_cuda
        self.device_ids = device_ids
        self.fc_input_shape = (mask_shape[0] // 16) * (mask_shape[1] // 16)

        self.dis_conv_module = DisConvModule(self.input_dim, self.cnum)
        self.linear = nn.Linear(self.cnum*4*self.fc_input_shape, 1)
...
class GlobalDis(nn.Module):
    def __init__(self, config, image_shape, use_cuda=True, device_ids=None):
        ...
        self.use_cuda = use_cuda
        self.device_ids = device_ids
        self.fc_input_shape = (image_shape[0] // 16) * (image_shape[1] // 16)
