...
def addFixedSquareMask(img, k):
    ...
    """
    masked_img = img.copy()
    x = random.sample(range(1, img.shape[0] - k -1), 1)[0]
    y = random.sample(range(1, img.shape[1] - k - 1), 1)[0]
    label_img = img[x:x+k, y:y+k, :]
    masked_img = cv2.rectangle(masked_img, pt1=(x, y), pt2=(x + k-1, y + k-1), color=(0, 0, 0), thickness=-1)
    return masked_img, y, x, label_img

if __name__ == "__main__":
    savepath = "./save"
    imgpath = "./data/b9.jpg"
    img = cv2.imread(imgpath)
    masked_img, x, y, label_img = addFixedSquareMask(img, 10)
    print(masked_img.shape)
    print(label_img.shape)
    print("Location: {}, {}", x, y)
    cv2.imwrite(os.path.join(savepath, os.path.basename(imgpath)), label_img)
